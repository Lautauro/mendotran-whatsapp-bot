...
export async function read_response(response: CommandReturn, message: Message): Promise<Message | void> {
    if (response.code === CommandResponse.OK) {
        ...
        if (response.data.content) {
            switch (response.type) {
                case CommandResponseType.REPLY_MESSAGE:
                    return await reply_message(response.data.content, message.id, { sendSeen: true, ...response.data.options});
                case CommandResponseType.SEND_MESSAGE:
                    return await send_message(response.data.content, message.id, { sendSeen: true, ...response.data.options});
            }
        }
        ...
        return await send_message(`⚠️ *Ha ocurrido un error* ⚠️\n\n${response.data.content}`, message.id, { sendSeen: true, ...response.data.options});
    }
}